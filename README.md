...
推荐链路：Jev 判断需求类型 → 程序用中文语义扩展、简介主题、内容类型、语言/类型/片名结构信号和质量加权从全库召回最多 500 部 → 每批 20 部、最多 25 批并行交给 Jev 独立评分 → 只展示得分达到阈值的最多 12 部。用户写“电影”时只召回电影，写“剧集、电视剧、美剧、动画剧”等时只召回剧集；没有明确类型时两者都可进入候选。未知片长不会通过片长限制，剧集的片长指单集时长。明确片名优先召回；「像某片」会排除参考片本身。固定评估需求与改动前后对比在 `data/eval/`。
