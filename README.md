...
正式片库来自 TMDB，覆盖多个原始语言，按热度并结合多语言、不同年代获取，优先收录票数至少 10、有海报和简介的电影。当前 26,252 部。这是精选覆盖，不是全球全量库。
...
推荐链路：Jev 判断需求类型 → 程序用中文语义扩展、简介主题、语言/类型/片名结构信号和质量加权从全库召回最多 100 部 → 每批 20 部、最多 5 批并行交给 Jev 独立评分 → 只展示得分达到阈值的最多 12 部。阈值是产品起始值，不代表经校准的喜欢概率。没有足够证据时允许空结果。未知片长不会通过片长限制。明确片名优先召回；「像某片」会排除参考片本身。固定评估需求与改动前后对比在 `data/eval/`。
...
Worker 必须验证访问码；CORS 不代替认证。Durable Object 保证共享上限 100 次新筛选/UTC 日；缓存命中不计额度。每次筛选最多 6 次 Jev 请求（1 次意图判断 + 5 次电影评分），不自动重试。服务错误明确展示，绝不伪造 AI 推荐。日志默认关闭，避免记录用户需求。

## 验证

`npm test` 检查片长约束、中文年代、召回、100 个候选全量评分、并发限制、生产分片与本地召回一致性、AI 响应校验及错误处理。真实 Jev 和浏览器验证记录见 `VERIFICATION.md`。
...
`node scripts/benchmark-recommend.mjs --live` 在同一份完整片库上运行三条固定需求，各测试一次 24 部（8 部/批、并发 3）和 100 部（20 部/批、并发 5），共六次无缓存的真实推荐。记录 token 用量、请求数和每批耗时，任一失败即停止，不自动重试；此命令会产生 Jev 调用费用。结果在 `data/eval/benchmark-100.json`。小样本用于实际速度观察，不代表稳定延迟保证或推荐质量普遍提升。18 条固定评估需求的 24/100 候选命中对比也记录在该文件。
...
本轮补充 3,956 部，总计 26,252 部，覆盖 73 种原始语言（含未知语言代码）。原有 22,296 部及其资料全部保留。新增资料验证未通过的 27 部未收录，网络失败 0。实际速度与证据边界见 [SPEED_REPORT.md](SPEED_REPORT.md)。
