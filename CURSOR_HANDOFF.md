...
当前继续执行用户指定的扩库及 1000 部候选方案。此前已完成固定评估基线、混合召回和最简单的看过/不合适反馈。不要重建项目、缩减片库或改成华语限定。不要顺带开发正片播放或更换 Jev。后续可改善多轮交互或开放心情句的漏召回，以用户最新指令为准。
...
- 当前内容库 9,574 部：7,197 部电影、2,374 部剧集；catalog version `3fdc62f30bca0a51`。
...
- 召回策略版本 `RECALL_VERSION=v5-1000`，已写入 Worker / 本地缓存键，避免命中旧算法的 24 小时缓存。
- 21 项测试通过。本轮同片库三条真实需求完成 500/1000 候选对照；详细证据在 VERIFICATION.md、SPEED_REPORT.md 与 `data/eval/benchmark-1000.json`。
...
3. `retrieve()` 从全库召回最多 **1000 部**：硬过滤 + 语言/类型/片名结构信号 + 简介主题词 + 有结构约束时的评分人数加权。生产通过 CANDIDATE_LIMIT=1000 指定上限。
4. 每批 20 部，最多 50 批并行交给 Jev 评分；得分 >=1.8，最多展示 12 部。每次最多 51 次 Jev API 调用。
5. 经典偏好只在明确提出且未否定时加权核实榜单作品。

瓶颈仍在：未进入 1000 部候选的内容不会被 Jev 评估。扩大候选数不等于已证明推荐质量提升，需要看固定评估与用户反馈；本轮三条需求的输入 token 约增加 1.9 倍，Jev 请求数从 26 次增至 51 次。开放心情句若简介没有对应词，合适片仍会漏掉。阈值不是校准后的喜欢概率。API 异常明确报错，不允许用规则结果冒充 Jev。
...
- 所有访客共享每天 100 次新筛选（UTC），缓存命中不扣额度；一次筛选最多 51 次模型请求。24 小时缓存。

网站独立运行于 GitHub Pages + Cloudflare。片库目前手动更新。保留 TMDB 署名和既有来源链接。

## 本轮测速方式

`node scripts/benchmark-recommend.mjs --live` 默认会产生六次无缓存的付费推荐（3 条需求 × 24/100 两档）；本轮 500/1000 对照记录在 `data/eval/benchmark-1000.json`。不自动重试。HTTP 请求不能覆盖 candidateLimit、batchSize 或 concurrency；这些参数仅供本地测速显式使用。
